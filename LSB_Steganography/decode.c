...
Status check_magic_string(DecodeInfo *decInfo)
{
    ...
    if(decode_byte_from_lsb_to_buffer(buffer, MAGIC_STRING_LENGTH * 8, magic_string, decInfo) == e_success)
    {
...
Status decode_data_size(DecodeInfo *decInfo)
{
    ...
    if(decode_num_from_lsb_to_int(buffer,&decInfo->data_size,decInfo) != e_success)
        return e_failure;
    printf("debug : %ld\n",decInfo->data_size);
...
Status decode_data_to_file(DecodeInfo *decInfo)
{
    ...
    if(decode_byte_from_lsb_to_buffer(src_buffer, decInfo->data_size* 8, dest_buffer, decInfo) == e_success)
    {
...
Status decode_out_file_extn_size(DecodeInfo *decInfo)
{
    ...
    if(decode_num_from_lsb_to_int(buffer,&decInfo->exten_out_file_size,decInfo) != e_success)
        return e_failure;
    printf("INFO : Extension file size extracted..\n");
    return e_success;
}
...
Status decode_out_file_extn(DecodeInfo *decInfo)
{
    ...
    if(decode_byte_from_lsb_to_buffer(buffer, MAX_FILE_SUFFIX * 8, decInfo->exten_out_file, decInfo) == e_success)
    {
        printf("debug : ");    
 ...
 */
Status decode_byte_from_lsb_to_buffer(const char * src_buffer, int src_buffer_size, char * dest_buffer, DecodeInfo *decInfo)
{
 ...
 * Return Value: e_success or e_failure, on file errors
 */
Status decode_num_from_lsb_to_int(const char * src_buffer, long int *extn_size, DecodeInfo *decInfo)
{
    int size = 0;
